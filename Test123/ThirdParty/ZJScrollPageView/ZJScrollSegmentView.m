...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX+self.lineSpace , self.zj_height - self.segmentStyle.scrollLineHeight, coverW-self.lineSpace*2 , self.segmentStyle.scrollLineHeight);
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.scrollLine.zj_x = currentTitleView.zj_x+self.lineSpace;
                weakSelf.scrollLine.zj_width = currentTitleView.zj_width- self.lineSpace*2;
            } else {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    weakSelf.scrollLine.zj_x = scrollLineX+self.lineSpace;
                    weakSelf.scrollLine.zj_width = scrollLineW-self.lineSpace*2;
                } else {
                    weakSelf.scrollLine.zj_x = currentTitleView.zj_x+self.lineSpace;
                    weakSelf.scrollLine.zj_width = currentTitleView.zj_width-self.lineSpace*2;
                }
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress+self.lineSpace;
            self.scrollLine.zj_width = oldTitleView.zj_width + wDistance * progress-self.lineSpace*2;
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                self.scrollLine.zj_x = oldScrollLineX + xDistance * progress+self.lineSpace;
                self.scrollLine.zj_width = oldScrollLineW + wDistance * progress-self.lineSpace*2;
            } else {
                self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress+self.lineSpace;
                self.scrollLine.zj_width = oldTitleView.zj_width + wDistance * progress-self.lineSpace*2;
            }
