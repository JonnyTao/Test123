...
//    NSLog([SJYYUtil LocalizedForKey:@"出现出现:current:---- %ld   old ----- %ld indexpathRow----%ld "], _currentIndex, _oldIndex, indexPath.row);
    if (_sysVersion >= 8) {
        [self setupChildVcForCell:cell atIndexPath:indexPath];
    }
...
//    NSLog([SJYYUtil LocalizedForKey:@"消失消失:current:---- %ld   old ----- %ld indexpathRow----%ld "], _currentIndex, _oldIndex, indexPath.row);
